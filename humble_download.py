...
class HumbleDownload(object):
    ...
    def md5_matches(self):
        """
            Determines whether the local MD5 matches the Humble Bundle specified MD5.

            :return:   True or False depending upon whether the MD5 checksums match.
            :rtype:  bool
        """
    ...
    def full_filename(self):
        """
            Calculates and returns the full path and filename for the current Humble Bundle download.

            :return:  The full path and filename for the current download.
        """
        return os.path.join(ConfigData.download_location, self.subproduct_name, self.platform, self.filename)
    ...
    def check_status(self):
        ...
        if not os.path.exists(self.full_filename):
            ...
            self.status_message = "%s file sizes don't match (expected %d actual %d)." % (self.filename,
            ...
            if not self.md5_matches:
                self.status_message = "MD5 of %s doesn't match (expected %s actual %s)." % (self.filename,
                                                                                            self.humble_md5,
                                                                                            self.local_md5)
                self.requires_download = True
        else:
    ...
    def __resume_download(self):
        ...
        web_request = requests.get(self.download_url, headers=resume_header, stream=True)
        mode = "ab"
    ...
    def __create_directory(self):
        """ Creates the directory for storing the current file if it doesn't exist. """
        full_directory = os.path.join(ConfigData.download_location, self.subproduct_name, self.platform)
        if not os.path.exists(full_directory):
            os.makedirs(full_directory)

    def is_valid(self):
        if self.humble_file_size is None or self.humble_file_size == 0:
            self.status_message = "Humble file size reported as 0.  Download is invalid."
            return False
        if self.download_url is None or len(self.download_url) == 0:
            self.status_message = "Humble download URL is an empty string.  Download is invalid."
            return False
        if self.humble_md5 is None or len(self.humble_md5) == 0:
            self.status_message = "Humble MD5 is an empty string.  Download is invalid."
            return False
        if self.filename is None or len(self.filename) == 0:
            self.status_message = "Filename is an empty string.  Download is invalid."
            return False
